...
if __name__ == '__main__':
    ...
    # exp number
    exp_number = '2' # sys.argv[1]

    # paths
    original_dataset_path = '../../../../../data/1/dataset_sequences.h5'
    ...
    for risk_level in range(1,nbr_images_in_seq):
        ...
        # visualize
        imageio.imwrite(os.path.join(save_path, 'train_pos.jpg'), x_train[0].astype(np.uint8))
        imageio.imwrite(os.path.join(save_path, 'train_neg.jpg'), x_train[nbr_seq * risk_level].astype(np.uint8))
        imageio.imwrite(os.path.join(save_path, 'valid_pos.jpg'), x_valid[0].astype(np.uint8))
        imageio.imwrite(os.path.join(save_path, 'valid_neg.jpg'), x_valid[nbr_seq * risk_level].astype(np.uint8))
